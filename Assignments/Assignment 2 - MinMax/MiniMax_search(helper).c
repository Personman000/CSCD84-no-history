...
//printf("min_urdl=[%f %f %f %f]\n",minmax_cost[12][22],minmax_cost[13][23],minmax_cost[12][24],minmax_cost[11][23]);
 ...
 return MiniMax_helper(gr, path, minmax_cost, cat_loc, cats, cheese_loc, cheeses, mouse_loc, mode, utility, agentId, depth, maxDepth, alpha, beta, -1);
}

double MiniMax_helper(double gr[graph_size][4], int path[1][2], double minmax_cost[size_X][size_Y], int cat_loc[10][2], int cats, int cheese_loc[10][2], int cheeses, int mouse_loc[1][2], int mode, double (*utility)(int cat_loc[10][2], int cheese_loc[10][2], int mouse_loc[1][2], int cats, int cheeses, int depth, double gr[graph_size][4]), int agentId, int depth, int maxDepth, double alpha, double beta, int source_neighbor_index)
{
...
//printf("n:%d, s:%d\n",neighbor_index, source_neighbor_index);
 	...
 	if(gr[ind][neighbor_index]==1 && source_neighbor_index != neighbor_index)
 	{
//printf("Checking!\n");
	 	// If agent is mouse
	 	if(agentId==0)
	 	{
			...
			// check utility of game state with new mouse position
		 	new_utility_value = MiniMax_helper(gr, path, minmax_cost, cat_loc, cats, cheese_loc, cheeses, new_mouse_loc, mode, utility, next_agentId, depth+1, maxDepth, alpha, beta, (neighbor_index+2)%4);
		...
		if(agentId>0)
		{
			...
			// check utility of game state with new cat position
		 	new_utility_value = MiniMax_helper(gr, path, minmax_cost, new_cat_loc, cats, cheese_loc, cheeses, mouse_loc, mode, utility, next_agentId, depth+1, maxDepth, alpha, beta, (neighbor_index+2)%4);
...
printf("curr_n_util=[");
	 ...
	 for(neighbor_index = 0; neighbor_index < 4; neighbor_index++)
	 {
		...
		if(curr_neighbor_util>=next_utility && gr[ind][neighbor_index]==1)
		{	
